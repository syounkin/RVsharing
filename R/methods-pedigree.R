...
setMethod("RVsharing",  signature(data="pedigree", dad.id="missing",mom.id="missing",carriers="missing"), function(data,carriers){
...
setMethod("RVsharing",  signature(data="character",dad.id="character",mom.id="character",carriers="missing"), function(data, dad.id, mom.id, carriers){
    # Converting 0s to NA  in dad.id and mom.id
    dad.id = ifelse(dad.id=="0",NA,dad.id)
    mom.id = ifelse(mom.id=="0",NA,mom.id)
...
setMethod("RVsharing",  signature(data="numeric",dad.id="numeric",mom.id="numeric",carriers="missing"), function(data, dad.id, mom.id, carriers){
    # Converting 0s to NA  in dad.id and mom.id
    dad.id = ifelse(dad.id==0,NA,dad.id)
    mom.id = ifelse(mom.id==0,NA,mom.id)
    RVsharing.fn( id = data, dad.id = dad.id, mom.id = mom.id)
