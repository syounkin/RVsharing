...
@
=== Inbreeding ===
<<RVsharing.toy4, eval = FALSE, echo = FALSE>>=
id <- paste0("sub", 1:9)
fa.id <- c(NA,NA,NA,NA, "sub1","sub1", "sub3","sub4","sub8")
ma.id <- c(NA,NA,NA,NA, "sub2","sub2", "sub5","sub6","sub7")
test.ped <- pedigree(id = id, dadid = fa.id, momid = ma.id, sex = c(1,2,1,1,2,2,2,1,1))
fa.id <- ifelse(is.na(fa.id),"0",fa.id)
ma.id <- ifelse(is.na(ma.id),"0",ma.id)
@ 
<<showped4, echo = FALSE, results = hide>>=
test.ped.second.cousins <- test.ped
test.ped
@ 

<<inbred, eval = FALSE, echo = FALSE>>=
trio1 <- new("Trio", id = "sub7", spouse = "sub8", offspring = list("sub9"))
trio2 <- new("Trio", id = "sub8", spouse = "sub7", offspring = list("sub9"))
trio3 <- new("Trio", id = "sub5", spouse = "sub3", offspring = list(trio1))
trio4 <- new("Trio", id = "sub6", spouse = "sub4", offspring = list(trio2))
trio5 <- new("Trio", id = "sub1", spouse = "sub2", offspring = list(trio3,trio4))

geno.vec <- c(1,0,0,0,rep(NA,5))
names(geno.vec) <- paste0("sub",1:9)
share.vec <- c()
occur.vec <- c()
for(i in 1:1e3 ){
      geno.vec.sim <- GeneDrop(trio5, geno.vec)
#      share.vec <- c(share.vec, geno.vec.sim["sub11"]==1 & geno.vec.sim["sub12"]==1)
      occur.vec <- c(occur.vec, geno.vec.sim["sub9"]==1)
}
sum(occur.vec)/1e3
@
